...
namespace ModulesGarden\Servers\ZimbraEmail\Core\App\Controllers\AppControllers;
...
class Addon extends \ModulesGarden\Servers\ZimbraEmail\Core\App\Controllers\AppController implements \ModulesGarden\Servers\ZimbraEmail\Core\App\Controllers\Interfaces\AppController
{
    public function getControllerInstanceClass($callerName, $params)
    {
        $functionName = str_replace($this->getModuleName() . "_", "", $callerName);
        $coreAddon = "\\ModulesGarden\\Servers\\ZimbraEmail\\Core\\App\\Controllers\\Instances\\Addon\\" . ucfirst($functionName);
        if (class_exists($coreAddon) && is_subclass_of($coreAddon, "ModulesGarden\\Servers\\ZimbraEmail\\Core\\App\\Controllers\\Instances\\AddonController")) {
            return $coreAddon;
        }
        $appAddon = "\\ModulesGarden\\Servers\\ZimbraEmail\\App\\Http\\Actions\\" . ucfirst($functionName);
        if (class_exists($appAddon) && is_subclass_of($appAddon, "ModulesGarden\\Servers\\ZimbraEmail\\Core\\App\\Controllers\\Instances\\AddonController")) {
            return $appAddon;
        }
        return NULL;
    }
